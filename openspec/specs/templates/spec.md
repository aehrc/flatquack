...
Defines the built-in template catalog and how a `@name` built-in template is resolved to a file,
including the requirement that the same set of built-in templates works for both the `struct` and
`staged` backends with minimal duplication.
